...
- ✅ 2026-08-10 修好 CI 上長期紅燈的 visual regression：比對改為「成塊差異」（差異遮罩做 5×5 形態學侵蝕），濾掉跨機器的文字抗鋸齒雜訊。實測雜訊由 1.22% 降到 0.034%（兩次獨立 CI 取樣），門檻收緊到 0.2%；並以「注入回歸」對照組驗過測試仍會紅（0.617%）。**已在 CI 驗收**：commit `5982b35` 的 `iOS Tests` run，Unit Tests 與 UI Tests 皆 success —— 連紅六次後第一次全綠。理由見 `DECISIONS.md` §15，症狀與排除過程見 `bugs.md` §11。
